...
void setup(){
  ...
  OuterRingScale = 1+PI*sqrt(3)/4.0/float(m);
...

  {
    ...
    for (Node=0;Node<=LastNode;Node++){
      Coord[Node][2]=zScale*(pow(Coord[Node][0],2)-pow(Coord[Node][1],2))/WidestInnerRadius;//Position in z axis
    }
  }
  {
    int Member = -1;
    ...
    for (Member=0;Member<=LastMember;Member++){
      if (Member<16*m){
        ...
        if (ConstantStiffness) L0[Member] = 1;
